...
exports.getLeads = async (req, res) => {
  try {
    const {
      status, source, city, state, productInterest,
      assignedTo, search, leadType, page = 1, limit = 20,
    } = req.query;

    const hasFilters = status || source || city || state || productInterest || assignedTo || search || leadType;
    ...
    if (productInterest) query = query.where("productInterest", "==", productInterest);
    ...
    const start = (parseInt(page) - 1) * parseInt(limit);
    res.json({ leads: leads.slice(start, start + parseInt(limit)), total, page: parseInt(page), limit: parseInt(limit) });
  } catch (err) {
...
exports.createLead = async (req, res) => {
  try {
    const { name, phone, email, source, city, state, productInterest, notes, companyName, leadType } = req.body;
    ...
    await db.collection("leads").doc(leadId).set({ ...lead, leadId });
...
exports.importLeads = async (req, res) => {
  try {
    const { leads, batchLeadType } = req.body;
    ...
    for (const l of leads) {
      ...
      batch.set(ref, {
        ...
        leadType:        l.leadType    || l.lead_type || batchLeadType || "",
...
exports.updateLead = async (req, res) => {
  try {
    const { id } = req.params;
    const allowed = ["name", "phone", "email", "companyName", "source", "city", "state", "productInterest", "notes"];
    const updates = {};
...
exports.assignLead = async (req, res) => {
  try {
    ...
    // Clear sales cache so the assigned user sees this lead immediately
    clearCachePrefix(`myLeads_${assignedTo}`);
...
exports.bulkAssign = async (req, res) => {
  try {
    ...
    await batch.commit();
    // Clear sales cache so the assigned user sees these leads immediately
    clearCachePrefix(`myLeads_${assignedTo}`);
    clearCachePrefix("teamLeads");
...
exports.importExternalLeads = async () => {
  ...
  try {
    ...
    for (const l of leads) {
      ...
      const existing = await db.collection("leads").where("sourceId", "==", externalIdStr).limit(1).get();
      if (!existing.empty) continue;

      const leadId = await getNextLeadId();
      const fullMessage = l.message || "";
      const cleanNotes = fullMessage.split("Sender Details")[0].replace(/Buyer is looking for\s*/i, "").trim();
      const emailMatch = fullMessage.match(/[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[A-Za-z]{2,}/);
      const email = emailMatch ? emailMatch[0] : l.sender_email || "";

      // TradeIndia provides company name in sender_company or company_name fields
      const companyName = l.sender_company || l.company_name || l.sender_organization || "";
    ...
    if (count > 0) console.log(`[TradeIndia] Success: ${count} new leads imported.`);
  } catch (err) {
